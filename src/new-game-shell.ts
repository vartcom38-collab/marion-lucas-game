...
function mount(){
 ...
 function renderPanel(){if(!state.tool){panel.hidden=true;return}panel.hidden=false;
  ...
  if(state.tool==='agenda'){const a=runtime().agenda; body.innerHTML=`<em>AGENDA · JOUR ${state.day}</em><h2>Ma journée</h2>${state.day===1?'<article><b>10:30 · Retrouver Marine</b><span>Centre-ville</span>':'<article><b>Matin · Temps pour toi</b><span>Nîmes</span>'}<div class="ng-inline"><button data-agenda="go">Y aller</button><button data-agenda="warn">Prévenir</button><button data-agenda="ignore">Continuer ici</button></div></article><article><b>Après-midi</b><span>Temps libre</span></article><article><b>Soirée</b><span>${a?.evening||'Rien de prévu… pour l’instant.'}</span></article>`;}
  if(state.tool==='map'){const places=runtime().places; body.innerHTML=`<em>CARTE · NÎMES</em><h2>Où aller ?</h2><div class="ng-map"><span class="road r1"></span><span class="road r2"></span><button class="pin home" data-place="Chez Marion">⌂<small>Chez Marion</small></button><button class="pin centre" data-place="Centre-ville">●<small>Centre</small></button><button class="pin arena" data-place="Arènes">●<small>Arènes</small></button></div><p class="ng-rule">Tu es actuellement : <b>${state.place}</b>. Tu choisis un lieu, jamais l’événement qui t’y attend.</p>${Array.isArray(places)?'<p class="ng-worldhint">'+places.length+' lieux actuellement compatibles avec cette partie.</p>':''}`;}

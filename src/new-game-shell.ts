...
function mount(){
 ...
 function renderPanel(){if(!state.tool){panel.hidden=true;return}panel.hidden=false;
  ...
  if(state.tool==='agenda') body.innerHTML=`<em>AGENDA · JOUR ${state.day}</em><h2>Ma journée</h2>${state.day===1?'<article><b>10:30 · Retrouver Marine</b><span>Centre-ville</span>':'<article><b>Matin · Temps pour toi</b><span>Nîmes</span>'}<div class="ng-inline"><button data-agenda="go">Y aller</button><button data-agenda="warn">Prévenir</button><button data-agenda="ignore">Continuer ici</button></div></article><article><b>Après-midi</b><span>Temps libre</span></article><article><b>Soirée</b><span>Rien de prévu… pour l’instant.</span></article>`;

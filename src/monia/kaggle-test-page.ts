...
const MARION='https://marion-lucas.marionbolomey.fr/resources/monia/canon/marion/reference.jpg';
...
function testJob(){
  ...
  return{
    ...
    sceneFamily:'isolated-test',signature:'manual-isolated-video-test',
    prompt:'Photorealistic cinematic test scene, isolated from the game story. Marion and Lucas are in a neutral interior, calm and natural. Preserve both canonical identities exactly. Only subtle breathing, blinking, tiny gaze changes and a restrained natural smile. No story event, no reveal, no text, no subtitles, no watermark, no UI.',
    ...
    characters:[{id:'marion',canonRef:MARION,wardrobe:'neutral contemporary outfit'},{id:'lucas',canonRef:LUCAS,wardrobe:'neutral contemporary outfit'}],
...
async function run(){
  ...
  try{
    ...
    for(let i=0;i<120;i++){
      ...
      if(result?.state!=='candidate'||result?.candidateOnly!==true||!Array.isArray(result?.clips)||result.clips.length<2)continue;
    ...
    throw new Error('Délai dépassé : la génération est peut-être encore en file Kaggle.');
  }catch(error){

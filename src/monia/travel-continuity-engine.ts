...
function durationMinutes(mode:TravelMode,from:string,to:string){const f=routeArea(from),t=routeArea(to);if(f===t)return f==='nimes'?25:35;if((f==='madrid'&&t==='finca')||(f==='finca'&&t==='madrid'))return90;if(mode==='flight')return360;if((f==='madrid'&&t==='salamanca')||(f==='salamanca'&&t==='madrid'))return150;if((f==='madrid'&&t==='seville')||(f==='seville'&&t==='madrid'))return330;if(['madrid','finca','seville','salamanca'].includes(f)&&['madrid','finca','seville','salamanca'].includes(t))return240;return90}
...
function transportMeta(owner:TravelPlan['owner'],mode:TravelMode){const lucasTrip=owner==='Lucas'||owner==='together';return{driver:mode==='flight'?'airport-transfer':lucasTrip?'chauffeur':'self' as TravelPlan['driver'],cuadrillaTruck:lucasTrip};}

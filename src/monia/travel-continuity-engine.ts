...
function estimateMode(from:string,to:string):TravelMode{const f=routeArea(from),t=routeArea(to);if(f===t)return'car';if((f==='nimes'&&['madrid','seville','salamanca'].includes(t))||(t==='nimes'&&['madrid','seville','salamanca'].includes(f)))return'flight';if((f==='madrid'&&['seville','salamanca'].includes(t))||(t==='madrid'&&['seville','salamanca'].includes(f)))return'train';return'car'}
function durationMinutes(mode:TravelMode,from:string,to:string){const f=routeArea(from),t=routeArea(to);if(f===t)return f==='nimes'?25:35;if((f==='madrid'&&t==='finca')||(f==='finca'&&t==='madrid'))return 90;if(mode==='flight')return 360;if(mode==='train'){if((f==='madrid'&&t==='salamanca')||(f==='salamanca'&&t==='madrid'))return 120;return 180;}if(['madrid','finca','seville','salamanca'].includes(f)&&['madrid','finca','seville','salamanca'].includes(t))return 150;return 90}
...
export function createTravelPlan(input:{owner:'Marion'|'Lucas'|'together';from:string;to:string;departDay?:number;departTime?:string;mode?:TravelMode;source:'calendar'|'choice'|'explicit'}){
  const s=read();if(!s)return null;const from=norm(input.from),to=norm(input.to);if(!from||!to||samePlace(from,to))return null;const departDay=input.departDay??n(s.day,1),departTime=input.departTime||String(s.time||'09:00');const mode=input.mode||estimateMode(from,to);const dur=durationMinutes(mode,from,to);const arr=addMinutes(departDay,departTime,dur);const hotelNight=arr.day>departDay||mins(arr.time)>=1320;const local=routeArea(from)===routeArea(to);const fatigue=local?3:mode==='flight'?18:mode==='train'?11:14;const plan:TravelPlan={id:`${input.owner}-${departDay}-${departTime}-${to}`.replace(/\s+/g,'-').toLowerCase(),owner:input.owner,from,to,mode,state:'planned',departDay,departTime,arrivalDay:arr.day,arrivalTime:arr.time,hotelNight,fatigue,source:input.source};const f=s.flags||(s.flags={});f.activeTravelPlan=plan;note(s,`travel-plan:${input.owner}:${from}->${to}`);write(s);return plan;
}

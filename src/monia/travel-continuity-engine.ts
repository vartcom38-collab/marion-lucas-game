...
export function finishTravelPlan(){const s=read();if(!s)return false;const plan=currentStored(s);if(!plan)return false;const f=s.flags||(s.flags={});delete f.activeTravelPlan;f.cuadrillaTruckOnRoute=false;f.lucasHasChauffeur=false;if(plan.owner==='Lucas'&&f.lucasCurrentPlace===plan.to)f.lucasWithMarion=false;note(s,`travel-finished:${plan.owner}:${plan.to}`);write(s);return true}

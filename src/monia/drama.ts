...
export type DramaShotSize = 'extreme-close' | 'close' | 'medium-close' | 'medium' | 'wide' | 'full' | 'two-shot' | 'detail';
...
const shotSizes = new Set<DramaShotSize>(['extreme-close','close','medium-close','medium','wide','full','two-shot','detail']);
...
export function parseDramaJSON(raw: string, fallback: MonIADramaPlan): MonIADramaPlan | null {
  ...
  try {
    ...
    const shots: MonIADramaShot[] = v.shots.slice(0,12).map((s:any, index:number) => ({
      ...
      generationPrompt: String(s?.generationPrompt || '').slice(0,1200),

...
export function getExtendedFamilyBeat():ExtendedFamilyBeat|null{
  ...
  const annual=getAnnualLifeProfile(),h=s.eventHistory||[],day=n(s.day,1),kids=n(s.children),slot=Math.floor(now/150);
  ...
  const seed=hash(`extended-family:${day}:${slot}:${annual?.lifeYear||1}:${evolution.mode}`)%100;
  ...
  const side:ExtendedFamilySide=(hash(`family-side:${day}:${annual?.lifeYear||1}`)%3===0?'both':hash(`family-side:${day}`)%2===0?'marion':'lucas');
  const sideLabel=side==='marion'?'de Marion':side==='lucas'?'de Lucas':'des deux côtés';

  add({id:`quiet-passage-${side}`,side,kind:'quiet-passage',label:'Profiter d’un passage familial tranquille',intent:`extended-family-quiet:${side}`,weight:annualWeight('home',56),minutes:45,narrative:`Un proche de la famille ${sideLabel} passe simplement un moment. Pas de grand programme : quelques nouvelles, la maison continue de vivre, puis chacun repart à son rythme.`,ordinary:true,rare:false},2);
  if(now>=690&&now<870||now>=1110&&now<1260)add({id:`meal-${side}`,side,kind:'family-meal',label:'Partager un repas avec la famille',intent:`extended-family-meal:${side}`,weight:annualWeight('home',63),minutes:85,narrative:`Un repas avec la famille ${sideLabel} s’insère naturellement dans la journée. Il peut être chaleureux, animé ou très simple sans devenir un événement exceptionnel.`,ordinary:true,rare:false},2);
  if(kids>0&&evolution.familyWeight>=52)add({id:`children-${side}`,side,kind:'child-support',label:'Laisser les proches profiter un peu des enfants',intent:`extended-family-children:${side}`,weight:annualWeight('home',68),minutes:75,narrative:`Avec les enfants, les proches prennent parfois davantage de place dans la maison. Ils passent voir la famille, donnent un coup de main ou restent un peu pendant que Marion et Lucas soufflent.`,ordinary:true,rare:false},1);
  if(evolution.mode==='open-house'&&energy>55)add({id:`longer-${side}`,side,kind:'stay-over',label:'Laisser la visite se prolonger',intent:`extended-family-stay:${side}`,weight:annualWeight('social',61),minutes:110,narrative:`La visite familiale se prolonge un peu. Rien n’oblige à en faire une réception : on parle, on mange peut-être quelque chose, les gens vont et viennent dans la maison.`,ordinary:true,rare:false},3);
  add({id:`check-before-${side}`,side,kind:'phone-before-visit',label:'Voir si la famille passe plus tard',intent:`extended-family-phone:${side}`,weight:annualWeight('social',44),minutes:10,narrative:`Un message ou un appel suffit parfois pour savoir si quelqu’un de la famille ${sideLabel} passera plus tard. La journée ne s’organise pas forcément autour de ça.`,ordinary:true,rare:false},2);
  if(!candidates.length)return null;
  return candidates[hash(`extended-family-pick:${day}:${slot}:${side}:${evolution.preferredZone}`)%candidates.length];
}

...
function toneInstruction(tone: MonIAMessageTone) {
  const instructions: Record<MonIAMessageTone, string> = {
    ...
    tender: 'Accueillir la tendresse sans devenir mièvre; Dominic peut être chaleureux et intime selon la relation.',
    ...
    jealous: 'Ne pas provoquer ni alimenter la jalousie. Dominic reste calme, fidèle et peut demander ce qui inquiète Marion.',
    hurt: 'Percevoir la blessure derrière une réponse froide; ne pas répondre sèchement ni dramatiser artificiellement.',
    angry: 'Ne pas escalader. Dominic peut poser une limite ou demander ce qui ne va pas, avec naturel.',
...
export function fallbackDirector(request: MonIADirectorRequest): MonIADirectorResult {
  const actor = request.actor || 'Dominic';

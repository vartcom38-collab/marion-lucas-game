...
type Message={from?:string;text?:string;read?:boolean;day?:number;thread?:string};
...
export function getTravelArrivalMoment():TravelArrivalMoment|null{
  ...
  return{id:`arrival-${plan.id}`,kind,title:returnMoment?.absenceDays&&returnMoment.absenceDays>=365?'De retour après longtemps':home?'De retour à la maison':familyBase?'Arrivée à la maison familiale':taurine?'Étape de tournée':hotel?'Arrivée à l’hôtel':'Arrivée',narrative,actions,luggage:!local,checkIn:hotel||taurine,cuadrillaSeparate:taurine,messageCue,returnAfterAbsence:returnMoment?{days:returnMoment.absenceDays,tone:returnMoment.tone,meaningful:returnMoment.meaningful}:undefined,residenceKind:residence.kind};
}

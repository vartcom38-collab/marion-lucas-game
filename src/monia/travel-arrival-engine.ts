...
function isHome(v:string){return/home|maison|appartement|madrid|finca|estate|family/.test(norm(v))}
function isHotel(v:string){return/hotel|hôtel/.test(norm(v))}
function isArenaTrip(p:TravelPlan){return p.source==='calendar'&&(p.owner==='Lucas'||p.owner==='together')}

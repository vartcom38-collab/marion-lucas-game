...
type Message={from?:string;thread?:string;text?:string;read?:boolean;day?:number};
...
function note(s:Save,e:string){s.eventHistory=[...(s.eventHistory||[]),e].slice(-220)}
function addMinutes(s:Save,minutes:number){const [h,m]=String(s.time||'09:00').split(':').map(Number);let total=(h||0)*60+(m||0)+minutes;while(total>=1440){total-=1440;s.day=n(s.day,1)+1}s.time=`${String(Math.floor(total/60)).padStart(2,'0')}:${String(total%60).padStart(2,'0')}`}
function isHotel(v:string){return/hotel|hôtel/.test(norm(v))}
function isArenaTrip(p:TravelPlan){return p.purpose==='taurine'&&(p.owner==='Lucas'||p.owner==='together')}
function isNimes(v:string){return/nîmes|nimes/.test(norm(v))}
function isEstate(v:string){return norm(v)==='estate'||/finca/.test(norm(v))}
function primaryJointFinca(){const snap=getPropertyLifeSnapshot();return snap.primary?.owner==='joint'&&snap.primary.kind==='finca'?snap.primary:null}

export function getTravelArrivalMoment():TravelArrivalMoment|null{
  ...
  return{id:`arrival-${plan.id}`,kind,title:returnMoment?.absenceDays&&returnMoment.absenceDays>=365?'De retour après longtemps':home?'De retour à la maison':familyBase?'De retour à la maison familiale':taurine?'Étape de tournée':hotel?'Arrivée à l’hôtel':'Arrivée',narrative,actions,luggage:!local,checkIn:hotel||taurine,cuadrillaSeparate:taurine,messageCue,returnAfterAbsence:returnMoment?{days:returnMoment.absenceDays,tone:returnMoment.tone,meaningful:returnMoment.meaningful}:undefined,residenceKind:residence.kind};
}
...
export function resolveTravelArrival(actionId:string){
  ...
  addMinutes(s,action.minutes);s.energy=Math.max(0,Math.min(100,n(s.energy,70)+action.energy));s.stress=Math.max(0,Math.min(100,n(s.stress)+action.stress));
  const f=s.flags||(s.flags={});f[`travelArrivalMomentDone:${plan.id}`]=true;f.lastArrivalKind=moment.kind;f.lastArrivalDay=n(s.day,1);f.lastArrivalPlace=plan.to;f.lastArrivalResidenceKind=moment.residenceKind;
  if(isSpainPlace(plan.to)&&f.spainLifeOpened!==true){f.spainLifeOpened=true;f.spainLifeOpenedDay=n(s.day,1);f.spainFirstStayPlace=plan.to;f.spainFirstStayResidenceKind=moment.residenceKind;note(s,`spain-life-opened:${plan.to}:${n(s.day,1)}`);}
  const finca=primaryJointFinca();if(isEstate(plan.to)&&finca){f.estateMoved=true;f.estateMovePending=false;f.estateMovePropertyId=finca.id;f.primaryPropertyId=finca.id;f.sharedHomePlace='estate';f.spainHomePlace='estate';f.spainHomeEstablished=true;f.spainHomeEstablishedDay=n(s.day,1);note(s,`estate-move-complete:${finca.id}:${n(s.day,1)}`);}
  if(moment.cuadrillaSeparate)f.cuadrillaArrivedSeparately=true;if(moment.returnAfterAbsence){f.lastReturnAbsenceDays=moment.returnAfterAbsence.days;f.lastReturnTone=moment.returnAfterAbsence.tone;}
  if(moment.messageCue==='lucas'&&!(s.messages||[]).some(m=>m.read===false&&norm(m.from)==='lucas'))s.messages=[...(s.messages||[]),{from:'Lucas',thread:'Lucas',text:'Bien arrivée ? Écris-moi quand tu es posée.',read:false,day:n(s.day,1)}];
  if(moment.messageCue==='marine'&&!(s.messages||[]).some(m=>m.read===false&&norm(m.from)==='marine'))s.messages=[...(s.messages||[]),{from:'Marine',thread:'Marine',text:'Alors, bien arrivée ? 🙂',read:false,day:n(s.day,1)}];
  note(s,`travel-arrival-moment:${moment.kind}:${actionId}`);write(s);if(moment.returnAfterAbsence)markPlaceReturn(plan.to);finishTravelPlan();return true;
}

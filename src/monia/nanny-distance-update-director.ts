...
export function maybeSendNannyDistanceUpdate(){
  const s=read();if(!s||n(s.children,0)<=0||!bothParentsAway(s)||!trustedNannyHasChildren(s))return null;const nanny=getTrustedNannySnapshot();if(!nanny?.available)return null;const f=s.flags||{},day=Math.max(1,n(s.day,1));if(n(f.trustedNannyLastUpdateDay,0)===day)return null;const t=mins(String(s.time||'12:00'));if(t<600||t>1260)return null;return queueTrustedNannyUpdate(desiredKind(s))
}

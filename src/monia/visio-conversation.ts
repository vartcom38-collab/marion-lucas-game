...
function responsePanel(){
  const overlay=document.getElementById('moniaVisioOverlay');
  if(!overlay)return null;
  let panel=overlay.querySelector<HTMLElement>('[data-monia-live-dialogue]');
  if(panel)return panel;
  panel=document.createElement('div');
  panel.dataset.moniaLiveDialogue='true';
...
function speak(text:string){if(!('speechSynthesis'in window))return;window.speechSynthesis.cancel();const u=new SpeechSynthesisUtterance(text);u.lang='fr-FR';u.rate=.96;u.pitch=.9;const voice=chooseVoice();if(voice)u.voice=voice;u.onstart=()=>status('Lucas parle');u.onend=()=>status('Lucas écoute');u.onerror=()=>status('Lucas écoute');window.speechSynthesis.speak(u)}

function contextFor(text:string){
  const save=readSave();if(!save)return null;
  const previous=lastVisio();
  ...
  return {
    speaker:'Marion',place:previous?.scene?.location||placeLabels[save.place]||save.place,time:save.time,day:save.day,
    recentAction:`Pendant une visio en cours, Marion dit à Lucas : ${text.slice(0,180)}`,
    activeObjective:'Poursuivre naturellement la visio en cours, répondre oralement à Marion sans sortir de la scène ni créer un événement futur',
    ...
    rules:['Ne jamais révéler un événement futur ou une surprise.','Lucas reste absolument fidèle.','Répondre comme Lucas dans une vraie visio, pas comme un assistant.','Ne jamais écrire la réponse de Marion.','Garder le lieu, la tenue et le contexte visuel de la visio actuelle sauf fait explicite contraire.','Réponse orale naturelle, brève, avec hésitations ou micro-pauses seulement si elles sonnent humaines.'],
  };
}

async function answerTurn(transcript:string){
  const context=contextFor(transcript);if(!context)return;
  const prefs=readPrefs();
  status('Lucas réfléchit…');
  setDialogue('Marion',transcript);
  try{
    ...
    try{sessionStorage.setItem(VISIO_KEY,JSON.stringify(result))}catch{}
    setDialogue('Lucas',result.spokenText||result.text);
    speak(result.spokenText||result.text);
  }catch(error){
    console.warn('[MonIA visio turn]',error);
    status('Lucas écoute');
  }
}
...
function installMic(){
  ...
  button.type='button';button.dataset.moniaVisioMic='true';button.textContent='🎙 Parler';
  ...
  button.onclick=()=>{
    try{window.speechSynthesis?.cancel();recognition?.abort?.();recognition=new Recognition();recognition.lang='fr-FR';recognition.interimResults=false;recognition.continuous=false;status('Marion parle…');button.textContent='● Écoute…';recognition.onresult=(e:any)=>{const text=String(e.results?.[0]?.[0]?.transcript||'').trim();if(text)void answerTurn(text)};recognition.onerror=()=>status('Lucas écoute');recognition.onend=()=>{button.textContent='🎙 Parler'};recognition.start()}catch{button.textContent='🎙 Parler';status('Lucas écoute')}
  };
...
console.info('[MonIA] Conversational visio voice turns active when browser speech recognition is available');

...
async function playV16(url:string,text:string,provider:string,duration?:number){
  ...
  audio.onplay=()=>{status('Lucas parle · V16');speechEvent('start',{provider,length:text.length,duration})};
  audio.onended=()=>{if(activeAudio===audio)activeAudio=null;status('Lucas réagit');speechEvent('end',{provider,duration})};
...
async function answerTurn(transcript:string){const context=contextFor(transcript);if(!context)return;const prefs=readPrefs();stopLucasAudio();stopGenericVoice();status('Lucas réfléchit…');speechEvent('thinking');setDialogue('Marion',transcript);try{const experience=await moniaExperience.respond({actor:'Lucas',playerText:transcript,requestedChannel:'visio',context,availableMedia:[]},prefs.aiMode as any,prefs.localAI!==false);const result=experience.response;try{sessionStorage.setItem(VISIO_KEY,JSON.stringify(result))}catch{}const line=result.spokenText||result.text;setDialogue('Lucas',line);void speak(line,result.emotion)}catch(error){console.warn('[MonIA visio turn]',error);status('Lucas écoute');speechEvent('error',{error:error instanceof Error?error.message:String(error)})}}
...
console.info('[MonIA] Conversational visio now requests, verifies and plays exact dynamic Lucas V16 audio; generic fallbacks remain forbidden');

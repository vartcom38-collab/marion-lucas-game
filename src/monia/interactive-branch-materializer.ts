...
import { bindCapturedContinuityFrame } from './generation-job';
...
function branchPrompt(input:MonIAExperienceResult,voiceDuration?:number){
  ...
  const continuity=input.generationJob.continuity.reference;
  const frameAuthority=continuity?.authority==='previous-rendered-frame'
    ? ' The supplied image is the authoritative previous rendered frame from the same scene. Continue from it directly; it has priority for pose, framing, room geometry, lighting, wardrobe, eyelines and visible props while canonical identity remains locked.'
    : '';
  ...
  return `Photorealistic live-action continuation of the exact previous frame.${frameAuthority} Do not reset or redesign the scene. Preserve the exact same canonical identities, face geometry, hair, wardrobe, visible tattoos, props, room geometry, lighting, eyelines, screen direction and physical positions unless the player's choice causally changes one of them. Actor focus: ${input.mediaPlan.actor}. Location: ${visual.location}. Framing: ${visual.framing}. Emotion: ${visual.emotion}. Continue action naturally: ${visual.action}.${speech} Natural breathing, blinking, small posture corrections and physically believable motion. No text, subtitles, watermark, morphing, identity drift or scene reset.`;
}
...
export async function materializeInteractiveBranchFromFrame(input:MonIAExperienceResult,referenceFile:File):Promise<MonIAMaterializedMedia>{
  const job=bindCapturedContinuityFrame(input.generationJob,referenceFile,'interactive-player-choice');
  let execution;
  try{execution=await executeMonIAGenerationJob(job)}catch{return {image:null,video:null,generationJobId:job.id,state:'voice-failed'}}
...
console.info('[MonIA] Interactive branch materializer active · captured frame is generation authority → V16 → speech-performance → next branch');

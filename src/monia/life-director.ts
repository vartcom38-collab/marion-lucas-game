...
export type LifeDirectorSnapshot={
  ...
  narrative:string;
  choices:LifeChoice[];
  freeActions:{phone:boolean;map:boolean;wardrobe:boolean;journal:boolean;customIntent?:boolean};
...
function surpriseBudget(s:Save){
  const daily=getDailyLifeSnapshot();const met=!!s.metLucas,unread=Number(s.phoneUnread||0)>0,stress=Number(s.stress||0),energy=Number(s.energy||100);const allowance=daily?.pacing.surpriseAllowance??1;
  return{voice:met&&energy>20&&allowance>0,cinematic:met&&stress<80&&allowance>0,call:met&&allowance>0,message:unread||met};
}
export function getLifeDirectorSnapshot():LifeDirectorSnapshot|null{
  ...
  return{day,time:String(s.time||'09:00'),place:String(s.place||'home'),age:ageFor(s),lifeYear:Math.floor((day-1)/365)+1,chapter:chapterFor(s),narrative:daily?.narrative||fallbackNarrative(s),choices,freeActions:{phone:true,map:true,wardrobe:true,journal:true,customIntent:true},surpriseBudget:surpriseBudget(s)};
}

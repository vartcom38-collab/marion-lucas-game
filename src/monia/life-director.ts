...
export function getLifeDirectorSnapshot():LifeDirectorSnapshot|null{
  ...
  return{day,time:String(s.time||'09:00'),place:String(s.place||'home'),age:ages.marionAge,lifeYear:ages.lifeYear,chapter:chapterFor(s),narrative,choices:choices.slice(0,5),freeActions:{phone:true,map:true,wardrobe:true,journal:true,customIntent:true},surpriseBudget:surpriseBudget(s)};
}

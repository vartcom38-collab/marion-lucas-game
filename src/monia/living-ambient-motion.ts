...
let ordinaryKind='';
let ordinaryMotion='';
let ordinaryTimer:number|undefined;
...
function mount(){
  ...
  root.className=`moniaAmbientMotion phase-${p} weather-${weather} season-${season}${ordinaryKind?` ordinary-${ordinaryKind}`:''}`;

...
type LooseSave={day?:number;time?:string;place?:string;relationship?:number;trust?:number;chemistry?:number;official?:boolean;metDominic?:boolean;memories?:string[];eventHistory?:string[];outfit?:string;flags?:Record<string,string|number|boolean>};
...
async function prepare(request:GameplayVisioRequest){
  ...
  try{
    ...
    const result=await moniaExperience.respond({
      actor:'Dominic',requestedChannel:'visio',
      context:{
        ...
        rules:[
          ...
          'Répondre comme Dominic dans une vraie visio, pas comme un assistant.',
          ...
          'Si Dominic est physiquement avec Marion, occupé ou indisponible, abandonner silencieusement la visio.'
        ],
      },availableMedia:[],
    },'auto',true);

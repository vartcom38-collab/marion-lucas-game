...
export type GameplayVisioMediaTicket={state:'ready';videoUrl:string;source:'approved-manifest';requestId:string;signature:string;authorizedAt:number;expiresAt:number};
...
async function prepare(request:GameplayVisioRequest){
  ...
  if(!hasApprovedLucasVisio()){console.warn('[Visio Director] no approved Lucas visio media is available yet');return}
  const clip=approvedLucasVisioFor('talk')||approvedLucasVisioFor('listen');
  if(!clip||!isApprovedLucasVisioSource(clip.src))return;
  ...
  try{
    ...
    const freshClip=approvedLucasVisioFor('talk')||approvedLucasVisioFor('listen');
    if(!freshClip||!isApprovedLucasVisioSource(freshClip.src))return;

...
export function getMarionHomeRhythm():MarionHomeRhythm|null{
  ...
  if(energy<32||stress>72)return{atHome:true,activity:'quiet-home',label:'Prendre un vrai moment pour moi',intent:'marion-home:quiet-home',kind:'rest',weight:84,minutes:55,independentFromLucas:true};
  ...
  const candidates:MarionHomeRhythm[]=[
    ...
    {atHome:true,activity:'self-care',label:'M’occuper un peu de moi',intent:'marion-home:self-care',kind:'self',weight:annualWeight('self',61),minutes:45,independentFromLucas:true},
    {atHome:true,activity:'reading',label:'Lire un moment tranquille',intent:'marion-home:reading',kind:'self',weight:annualWeight('home',54),minutes:40,independentFromLucas:true},
    {atHome:true,activity:'cooking',label:'Préparer quelque chose à manger',intent:'marion-home:cooking',kind:'self',weight:annualWeight('home',58),minutes:50,independentFromLucas:true},
    {atHome:true,activity:'work-project',label:'Avancer sur quelque chose à moi',intent:'marion-home:work-project',kind:'self',weight:annualWeight('career',63),minutes:65,independentFromLucas:true},
    ...
    {atHome:true,activity:'quiet-home',label:'Profiter simplement de la maison',intent:'marion-home:quiet-home',kind:'self',weight:annualWeight('home',57),minutes:35,independentFromLucas:true}
  ];

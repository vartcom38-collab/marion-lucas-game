...
type Save={screen?:string;overlay?:unknown;metDominic?:boolean;official?:boolean;place?:string;flags?:Record<string,unknown>};
...
function openLucasCall(){const s=read();if(!canDominicUsePhoneAutonomously(s))return false;return openPhone('call')}

function intimacyOverlay(){
  const s=read();if(!hasMetDominic(s)||!s?.official)return false;
...
function routeFreeText(text:string,kind:string){
  ...
  if(kind==='phone'||kind==='social'||/marine|appel|appeler|visio|sms|message|téléphone|telephone/.test(t)){
    if(/appel|appeler|visio/.test(t)&&/dominic|lucas/.test(t))return openLucasCall();
  ...
  if(kind==='relationship'&&/dominic|lucas|ensemble|moment à deux|moment a deux/.test(t))return intimacyOverlay()||openLucasCall();
  return openActions();
}

...
export function getChildcareSnapshot(context:ChildcareContext='home'):ChildcareSnapshot|null{
  ...
  const planned=plannedFor(s,context);const travel=context==='travel'||context==='torero-travel';const family=!travel&&familyAvailable(s);const nanny=planned||nannyAvailable();const available=planned||family||nanny;
  ...
  const reason=available?(mode==='family-support'?'La famille peut prendre le relais aujourd’hui. Ce soutien existe, mais il n’est pas supposé disponible à chaque sortie.':travel?(load.travelSensitive?'La nounou de confiance prend le relais pour les plus jeunes pendant le déplacement.':'La nounou de confiance peut prendre le relais pendant le déplacement.'):'La nounou de confiance est disponible. Marion et Lucas peuvent sortir sans que la garde devienne un obstacle artificiel.'):(travel?'Aucune solution de garde fiable n’est organisée aujourd’hui.':'Aucune garde fiable n’est organisée pour ce créneau.');

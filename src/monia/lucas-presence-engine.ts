...
function isProfessional(i:CalendarItem){return/corrida|feria|toros|arène|arena|entraînement|entrainement|presse|media|déplacement|deplacement|voyage|flight|route|meeting/i.test(`${i.title||''} ${i.note||''}`)}
...
export function getLucasPresence():LucasPresenceSnapshot|null{
  ...
  const item=currentItem(s);if(item){const lp=itemPlace(item);const la=area(lp);const professional=isProfessional(item);if(professional){const traveling=/déplacement|deplacement|voyage|flight|route/i.test(`${item.title||''} ${item.note||''}`);return{state:traveling?'traveling':'working',lucasPlace:lp||'activité professionnelle',marionPlace,together:false,reachableByPhone:!traveling,privateTimePossible:false,reason:traveling?'Lucas est en déplacement professionnel.':'Lucas a une activité professionnelle en cours.',source:'calendar'};}const together=la!=='unknown'&&la===marionArea;return homeAdjusted({state:together?'with-marion':la===marionArea?'same-area':'away',lucasPlace:lp||'agenda Lucas',marionPlace,together,reachableByPhone:true,privateTimePossible:together,reason:together?'L’agenda place Lucas au même endroit que Marion.':'L’agenda place Lucas ailleurs pour ce moment.',source:'calendar'});}
  if(f.travelWithLucas===true)return homeAdjusted({state:'with-marion',lucasPlace:marionPlace,marionPlace,together:true,reachableByPhone:true,privateTimePossible:true,reason:'Marion a choisi de voyager avec Lucas et aucun engagement séparé ne le place ailleurs.',source:'travel-choice'});

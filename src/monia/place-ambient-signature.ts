...
function remove(){document.getElementById('moniaPlaceAmbient')?.remove();document.documentElement.removeAttribute('data-monia-place-kind');document.documentElement.removeAttribute('data-monia-place-history');document.documentElement.removeAttribute('data-monia-place-life')}
function mount(){const s=read();if(!s||!eligible(s)){remove();return}const kind=placeKind(String(s.place||'home')),seasonal=getSeasonalLifeSnapshot(),history=getCurrentPlaceHistory(),evolution=getCurrentPlaceLifeEvolution();let root=document.getElementById('moniaPlaceAmbient');if(!root){root=document.createElement('div');root.id='moniaPlaceAmbient';root.className='moniaPlaceAmbient';root.setAttribute('aria-hidden','true');document.body.appendChild(root)}root.className=`moniaPlaceAmbient place-${kind} weather-${seasonal?.weather||'clear'} history-${history?.tier||'new'} life-${evolution?.mode||'fresh'}`;root.style.setProperty('--monia-place-density',String((evolution?.ambientDensity||40)/100));root.innerHTML=`<span class="placeGlow"></span><span class="placeTexture"></span><span class="placeMotion a"></span><span class="placeMotion b"></span>`;document.documentElement.dataset.moniaPlaceKind=kind;document.documentElement.dataset.moniaPlaceHistory=history?.tier||'new';document.documentElement.dataset.moniaPlaceLife=evolution?.mode||'fresh'}

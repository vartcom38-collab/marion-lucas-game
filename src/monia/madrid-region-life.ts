...
const FAMILY_VIEWS={
  arrival:'./resources/madrid/family-home-exterior.webp',
...
function syncFamilyVisual(s:Save){if(s.place!=='family')return;const view=familyView(s),src=FAMILY_VIEWS[view];const photo=document.querySelector<HTMLImageElement>('img.worldPhoto,.worldPhoto img,img[data-world-photo]');if(photo&&photo.dataset.familyView!==view){photo.dataset.familyView=view;photo.src=src;photo.alt='Maison familiale près de Madrid'}const background=document.querySelector<HTMLElement>('.worldPhoto:not(img),.worldBackdrop,.worldSceneBackground');if(background&&background.dataset.familyView!==view){background.dataset.familyView=view;background.style.backgroundImage=`url("${src}")`}}

...
type Save={day?:number;time?:string;place?:string;energy?:number;stress?:number;relationship?:number;flags?:Record<string,unknown>};
...
const ACTIONS={
  courtyard:{label:'Prendre un moment dans la cour',minutes:35,energy:-2,stress:-7,relationship:1,care:1,comfort:2,identity:1},
  living:{label:'S’installer dans le salon',minutes:45,energy:1,stress:-6,relationship:1,care:0,comfort:3,identity:2},
  kitchen:{label:'Préparer quelque chose ensemble',minutes:70,energy:-4,stress:-3,relationship:3,care:2,comfort:2,identity:2},
  grounds:{label:'Faire le tour des terres',minutes:80,energy:-7,stress:-4,relationship:1,care:4,comfort:0,identity:2},
  annex:{label:'S’occuper des dépendances',minutes:95,energy:-9,stress:1,relationship:1,care:6,comfort:1,identity:2},
} as const;
...
let raf=0;function schedule(){cancelAnimationFrame(raf);raf=requestAnimationFrame(ensureEntry)}window.addEventListener('monia:property-changed',schedule as EventListener);window.addEventListener('monia:finca-home-changed',schedule as EventListener);window.addEventListener('storage',schedule);new MutationObserver(schedule).observe(document.body,{childList:true,subtree:true});schedule();

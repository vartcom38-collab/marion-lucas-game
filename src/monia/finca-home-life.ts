...
type FincaState={version:1;propertyId?:string;settledDay?:number;care:number;comfort:number;identity:number;lastActionDay?:number;lastDecayDay?:number;visits:number;upgrades:string[];spaceUses:Record<FincaSpace,number>};
...
function dayOf(save:Save|null){return Math.max(1,Number(save?.day||1)||1)}
function addMinutes(save:Save,minutes:number){const current=String(save.time||'09:00').split(':').map(Number);let total=(current[0]||9)*60+(current[1]||0)+minutes;while(total>=1440){total-=1440;save.day=dayOf(save)+1}save.time=`${String(Math.floor(total/60)).padStart(2,'0')}:${String(total%60).padStart(2,'0')}`}
function syncProperty(state:FincaState){const finca=currentFinca(),save=readSave();if(!finca||!save)return null;if(state.propertyId!==finca.id){state.propertyId=finca.id;state.settledDay=dayOf(save);state.lastDecayDay=dayOf(save);state.care=18;state.comfort=20;state.identity=12;state.visits=0;state.upgrades=[];state.spaceUses={...DEFAULT.spaceUses};writeState(state)}return finca}
function applyPassiveEvolution(state:FincaState,save:Save){
  const today=dayOf(save);const baseline=state.lastDecayDay??state.settledDay??today;const elapsed=Math.max(0,today-baseline);if(elapsed<3)return false;
  const cycles=Math.floor(elapsed/3);const quietDays=state.lastActionDay?Math.max(0,today-state.lastActionDay):elapsed;
  if(quietDays>=3)state.care=clamp(state.care-Math.min(12,cycles*2));
  if(state.comfort>=40&&quietDays<=6)state.comfort=clamp(state.comfort+Math.min(2,cycles));
  state.lastDecayDay=baseline+cycles*3;
  return writeState(state);
}

export function getFincaHomeSnapshot(){const state=readState(),finca=syncProperty(state),save=readSave();if(finca&&save)applyPassiveEvolution(state,save);return{active:Boolean(finca),property:finca,state};}
...
function currentViewKey(save:Save,fincaName:string){const views=FINCA_VIEWS[fincaName];if(!views)return null;const last=save.flags?.lastFincaHomeAction;if(last==='courtyard'||last==='living'||last==='kitchen'||last==='grounds'||last==='annex')return views[last];return views.hero}
function syncEstateVisual(){const save=readSave(),finca=currentFinca();if(!save||save.place!=='estate'||!finca)return;const key=currentViewKey(save,finca.name);if(!key)return;const src=PROPERTY_VISUAL_ASSETS[key]||`./resources/properties/${key}.webp`;const photo=document.querySelector<HTMLImageElement>('img.worldPhoto,.worldPhoto img,img[data-world-photo]');if(photo&&photo.dataset.fincaViewKey!==key){photo.dataset.fincaViewKey=key;photo.src=src;photo.alt=finca.name}const background=document.querySelector<HTMLElement>('.worldPhoto:not(img),.worldBackdrop,.worldSceneBackground');if(background&&background.dataset.fincaViewKey!==key){background.dataset.fincaViewKey=key;background.style.backgroundImage=`url("${src}")`}}
function renderSceneActions(){if(!shouldShowSceneActions()){removeSceneActions();return}const save=readSave();if(!save)return;const state=readState();syncProperty(state);applyPassiveEvolution(state,save);let root=document.getElementById('moniaFincaSceneActions');if(!root){root=document.createElement('div');root.id='moniaFincaSceneActions';root.className='fincaSceneActions';sceneHost().appendChild(root)}const spaces=contextualSpaces(save,state);const mood=state.care<18?'À entretenir':state.identity>=45?'Chez eux':state.comfort>=40?'Ils prennent leurs habitudes':'Encore nouveau';root.innerHTML=`<div class="fincaSceneStatus"><span>${esc(mood)}</span><small>${esc(String(save.time||''))}</small></div><div class="fincaSceneButtons">${spaces.map(space=>`<button data-finca-scene-space="${space}" title="${esc(ACTIONS[space].label)}"><b>${esc(ACTIONS[space].short)}</b><small>${ACTIONS[space].minutes} min</small></button>`).join('')}<button class="fincaSceneMore" data-finca-scene-more title="Voir toute la finca"><b>•••</b><small>Maison</small></button></div>`;root.querySelectorAll<HTMLButtonElement>('[data-finca-scene-space]').forEach(btn=>btn.onclick=e=>{e.stopPropagation();const space=btn.dataset.fincaSceneSpace as FincaSpace;if(doFincaHomeAction(space)){syncEstateVisual();renderSceneActions()}});root.querySelector<HTMLButtonElement>('[data-finca-scene-more]')?.addEventListener('click',e=>{e.stopPropagation();openFincaHome()})}

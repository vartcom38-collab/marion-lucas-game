...
export type CommitmentReadiness={readiness:Readiness;proposalAllowed:boolean;canSignalReady:boolean;cooldownUntilDay:number;reason:string};
...
export function getCommitmentReadiness():CommitmentReadiness|null{
  ...
  const officialDay=chronology?.officialDay||day;const cooldown=n(f.proposalCooldownUntilDay,0);const estateDay=n(f.spainHomeEstablishedDay||f.lastArrivalDay,0);
  const sharedLifeReady=f.firstJointPropertyPurchased===true&&f.estateMoved===true&&estateDay>0&&day>=estateDay+7;
  const base=!!s.official&&!s.engaged&&!s.married&&n(s.relationship)>=68&&n(s.trust)>=58&&day>=officialDay+90&&sharedLifeReady;
  const proposalAllowed=base&&readiness!=='not-yet'&&day>=cooldown;
  const reason=proposalAllowed?'La relation, le temps et leur vie commune rendent une demande crédible, sans imposer sa date.':readiness==='not-yet'?'Marion a indiqué que ce serait trop tôt pour l’instant.':!sharedLifeReady?'Leur vie commune doit encore avoir le temps de s’installer avant d’ouvrir cette étape.':'La fenêtre n’est pas encore mûre.';
  return{readiness,proposalAllowed,canSignalReady:!!s.official&&!s.engaged&&!s.married&&sharedLifeReady,cooldownUntilDay:cooldown,reason};
}

export function setMarriageReadiness(value:Readiness){const s=read();if(!s)return false;const current=getCommitmentReadiness();if(!current?.canSignalReady)return false;ensureLifeMilestoneChronology(s);const f=s.flags||(s.flags={});f.marriageReadiness=value;f.marriageReadinessDay=Math.max(1,n(s.day,1));if(value==='ready')f.proposalCooldownUntilDay=0;write(s);window.dispatchEvent(new CustomEvent('monia:marriage-readiness',{detail:{value}}));return true}

export function recordProposalResponse(response:'yes'|'too-early'|'no-marriage'){
  const s=read();if(!s)return false;const current=getCommitmentReadiness();if(!current?.proposalAllowed)return false;ensureLifeMilestoneChronology(s);const f=s.flags||(s.flags={});const day=Math.max(1,n(s.day,1));

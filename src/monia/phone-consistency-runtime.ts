...
function terminal(state:string){return['ended','missed','unreachable','local','cancelled'].includes(state)}
function minute(t:string){const [h,m]=String(t||'0:0').split(':').map(Number);return(h||0)*60+(m||0)}
function sameCall(a:Row,b:Row){if(a.person!==b.person||a.kind!==b.kind||a.day!==b.day)return false;if(terminal(a.state)&&terminal(b.state))return false;return Math.abs(minute(a.time)-minute(b.time))<=2}
...
function syncPhoneDom(){const policy=getLucasCommunicationPolicy();const phone=document.querySelector<HTMLElement>('.phoneDevice');if(!phone)return;phone.dataset.lucasPhoneMode=policy.mode;phone.dataset.lucasPhoneLabel=policy.label;
  document.querySelectorAll<HTMLButtonElement>('[data-social-call="lucas"]').forEach(btn=>{if(policy.mode==='local'){btn.disabled=true;btn.textContent='Avec toi';btn.title='Lucas est physiquement avec Marion';}else{btn.disabled=false;btn.textContent='Appeler';btn.title=policy.label;}});
  document.querySelectorAll<HTMLElement>('.socialProfileHero').forEach(hero=>{const name=hero.querySelector('h3')?.textContent?.trim();if(name!=='Lucas')return;hero.dataset.lucasPhoneMode=policy.mode;});
  document.querySelectorAll<HTMLElement>('.nativeThreadHead').forEach(head=>{const name=head.querySelector('strong')?.textContent?.trim();if(name!=='Lucas')return;const status=head.querySelector<HTMLElement>('.nativePresence');if(status)status.textContent=policy.label;const call=head.querySelector<HTMLButtonElement>('button[aria-label="Appeler"]');if(call){call.disabled=policy.mode==='local';call.title=policy.mode==='local'?'Lucas est avec toi':policy.label;}});
}

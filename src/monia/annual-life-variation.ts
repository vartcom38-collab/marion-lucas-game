...
function currentLifeYear(s?:Save|null){return getLifeAgeSnapshot(s).lifeYear}

export function getAnnualLifeProfile():AnnualLifeProfile|null{
  const s=read();if(!s)return null;const year=currentLifeYear(s);const a=hash(`annual:${year}`)%TONES.length;let b=hash(`secondary:${year}`)%TONES.length;if(b===a)b=(b+1)%TONES.length;
...
export function annualBeatAllowed(id:string,opts:{recurring?:boolean;feria?:boolean;cooldownYears?:number}={}){
  ...
  const s=read();if(!s)return true;const year=currentLifeYear(s);const key=normalizedBeat(id);const history=s.eventHistory||[];const cooldown=Math.max(1,opts.cooldownYears??(opts.recurring?1:3));
  for(let y=Math.max(1,year-cooldown);y<=year;y++)if(history.some(e=>e===`annual-beat:${y}:${key}`))return false;
  return true;
}
export function markAnnualBeat(id:string,opts:{feria?:boolean}={}){const s=read();if(!s)return false;if(opts.feria)return true;const year=currentLifeYear(s);const key=normalizedBeat(id);s.eventHistory=[...(s.eventHistory||[]),`annual-beat:${year}:${key}`].slice(-260);localStorage.setItem(SAVE_KEY,JSON.stringify(s));window.dispatchEvent(new CustomEvent('monia:save-changed',{detail:{key:SAVE_KEY}}));return true}

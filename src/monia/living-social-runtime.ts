...
export function getLivingPresence():Presence|null{
 ...
 if(togetherWithLucas(s))return{id:'lucas',name:'Lucas',label:isLucasBusy(s)?'Occupé':'Il est vraiment là',detail:s.place==='madrid'?'Vous êtes au même endroit. Le téléphone ne remplace pas ce moment.':'Sa présence suit le lieu et son agenda.',action:'Passer un moment ensemble',minutes:45};
...
export function doLivingSocialMoment(actor:ActorId){const s=read();if(!s)return false;const p=getLivingPresence();if(!p||p.id!==actor)return false;addMinutes(s,p.minutes);const f=flags(s);if(actor==='marine'){s.stress=clamp(n(s.stress,10)-3);f.livingSocialMarineDay=n(s.day,1);f.socialCircle=n(f.socialCircle,0)+1;remember(s,'Un moment simple avec Marine a trouvé sa place dans une journée normale.')}else if(actor==='lucas'){s.relationship=clamp(n(s.relationship,0)+2);s.trust=clamp(n(s.trust,0)+1);s.stress=clamp(n(s.stress,10)-3);f.livingSocialLucasSharedDay=n(s.day,1);remember(s,'Un moment partagé avec Lucas a compté parce qu’ils étaient réellement au même endroit.')}else{s.stress=clamp(n(s.stress,10)-4);s.energy=clamp(n(s.energy,70)-1);f.livingSocialFamilyDay=n(s.day,1);remember(s,'La maison familiale a accueilli un moment ordinaire, sans occasion spéciale.')}return write(s)}

...
function togetherWithLucas(){return getLucasPresence()?.together===true}
function marineNearby(s:Save){const place=String(s.place||'');if(!['nimes','cafe','arenes'].includes(place))return false;const p=getSocialPresence('Marine');if(p?.status==='with-marion'&&p.place===place)return true;if(p?.status==='nearby'&&(p.place===place||p.place==='nimes'||p.place==='arenes'))return true;const day=n(s.day,1),f=flags(s),last=n(f.livingSocialMarineDay,-99);if(day-last<3)return false;const social=n(f.socialCircle,0)+n(p?.momentCount,0);const appears=social>0&&hash(`marine:${day}:${place}`)%5===0;if(appears)setSocialPresence('Marine',{status:'nearby',place,source:'living-world'},day,String(s.time||'12:00'));return appears}
function familyActive(s:Save){return s.place==='family'}

export function getLivingPresence():Presence|null{
 const s=read();if(!s||s.screen&&s.screen!=='game')return null;const h=Math.floor(mins(String(s.time||'12:00'))/60),lucas=getLucasPresence();
 ...
 if(lucas?.together)return{id:'lucas',name:'Lucas',label:'Il est vraiment là',detail:s.place==='madrid'?'Vous êtes au même endroit. Le téléphone ne remplace pas ce moment.':lucas.reason||'Sa présence suit le lieu et son agenda.',action:'Passer un moment ensemble',minutes:45};
...
function maybeSeedAmbientMessage(s:Save){const f=flags(s),day=n(s.day,1),place=String(s.place||''),lucas=getLucasPresence();if(day<2)return false;if(lucas?.together)return false;let changed=false;
 ...
 const lucasLast=n(f.livingSocialLucasSmsDay,-99);if(s.metLucas&&lucas?.state!=='working'&&lucas?.state!=='traveling'&&day-lucasLast>=3&&!['madrid','family','estate'].includes(place)&&hash(`lucas-sms:${day}:${place}`)%5===0){const pool=s.official?['Journée chargée ici. Écris-moi quand tu veux.','Je pense à toi. On se parle plus tard ?','Tu me racontes ta journée quand tu peux ?']:['Bonne journée :)','Tu fais quoi aujourd’hui ?','Je repensais à notre dernière conversation.'];const text=pool[hash(`lucas-copy:${day}`)%pool.length];if(pushIncoming(s,'Lucas',text)){f.livingSocialLucasSmsDay=day;changed=true}}
 return changed;
}

export function doLivingSocialMoment(actor:ActorId){const s=read();if(!s)return false;const p=getLivingPresence();if(!p||p.id!==actor)return false;addMinutes(s,p.minutes);const f=flags(s);if(actor==='marine'){s.stress=clamp(n(s.stress,10)-3);f.livingSocialMarineDay=n(s.day,1);f.socialCircle=n(f.socialCircle,0)+1;f.lastSocialPerson='Marine';f.lastSocialPlace=String(s.place||'');setSocialPresence('Marine',{status:'with-marion',place:String(s.place||''),source:'living-social-moment'},n(s.day,1),String(s.time||'12:00'));remember(s,'Un moment simple avec Marine a trouvé sa place dans une journée normale.')}else if(actor==='lucas'){if(!getLucasPresence()?.together)return false;s.relationship=clamp(n(s.relationship,0)+2);s.trust=clamp(n(s.trust,0)+1);s.stress=clamp(n(s.stress,10)-3);f.livingSocialLucasSharedDay=n(s.day,1);f.lastSharedLucasPlace=String(s.place||'');f.lastSharedLucasTime=String(s.time||'');remember(s,'Un moment partagé avec Lucas a compté parce qu’ils étaient réellement au même endroit.')}else{s.stress=clamp(n(s.stress,10)-4);s.energy=clamp(n(s.energy,70)-1);f.livingSocialFamilyDay=n(s.day,1);f.familyOrdinaryMoments=n(f.familyOrdinaryMoments,0)+1;f.lastFamilyPlace=String(s.place||'family');remember(s,'La maison familiale a accueilli un moment ordinaire, sans occasion spéciale.')}return write(s)}
...
console.info('[Living social] canonical Lucas presence now drives calls, local moments and visible world cast');

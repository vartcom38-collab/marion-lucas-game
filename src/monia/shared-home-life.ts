...
type Save={day?:number;time?:string;place?:string;metDominic?:boolean;official?:boolean;relationship?:number;trust?:number;energy?:number;stress?:number;flags?:Record<string,unknown>};
...
export function getSharedHomeBeat():SharedHomeBeat|null{
  const s=read();if(!s||!hasMetDominic(s)||!s.official)return null;const lucas=getLucasHomeRhythm(),marion=getMarionHomeRhythm();if(!lucas?.atHome||!marion?.atHome)return null;
  ...
  if(lucas.activity==='resting'||energy<45)add({id:'quiet-nearby',label:'Rester tranquillement dans le même espace',intent:'shared-home-rest',kind:'rest',weight:annualWeight('couple',60),minutes:45,narrative:'Dominic récupère et Marion peut rester près de lui sans que cela devienne une conversation ou une scène romantique. Juste partager le calme.',ordinary:true,romanceRequired:false},2);

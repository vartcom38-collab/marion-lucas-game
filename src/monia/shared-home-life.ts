...
export function getSharedHomeBeat():SharedHomeBeat|null{
  ...
  if(!candidates.length)return null;const seed=hash(`${day}:${Math.floor(now/90)}:${lucas.activity}:${marion.activity}:${annual?.tone||'normal'}`);return candidates[seed%candidates.length];
}

...
function meta(asset:MonIAAsset){const entries=Object.entries(asset.metadata||{}).filter(([,v])=>v!==null&&v!==undefined).slice(0,6);return entries.length?`<small style="display:block;opacity:.72;margin-top:8px">${entries.map(([k,v])=>`${safe(k)}: ${safe(String(v))}`).join(' · ')}</small>`:'';}

export function mountMonIAVaultReview(){
  if(document.getElementById('moniaVaultReview'))return;
  const host=document.createElement('section');host.id='moniaVaultReview';host.style.cssText='margin:24px auto;max-width:1100px;padding:22px;border:1px solid rgba(255,255,255,.12);border-radius:22px;background:rgba(20,18,16,.78);color:inherit';
  ...
  host.addEventListener('click',async e=>{const target=e.target as HTMLElement;const approve=target.closest('[data-vault-approve]') as HTMLElement|null;const reject=target.closest('[data-vault-reject]') as HTMLElement|null;if(approve){const id=approve.dataset.vaultApprove||'';if(id&&window.confirm('Valider ce média pour que MonIA puisse le réutiliser dans le jeu ?')){await moniaCreativeVault.updateAsset(id,{status:'approved'});await render();window.dispatchEvent(new CustomEvent('monia:vault-changed'));}return;}if(reject){const id=reject.dataset.vaultReject||'';if(id){await moniaCreativeVault.updateAsset(id,{status:'rejected'});await render();window.dispatchEvent(new CustomEvent('monia:vault-changed'));}}});
  document.getElementById('refreshMoniaVault')?.addEventListener('click',()=>void render());window.addEventListener('monia:vault-changed',()=>void render());void render();
}

if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',mountMonIAVaultReview,{once:true});else mountMonIAVaultReview();

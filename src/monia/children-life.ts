...
function canonicalBirthOpen(s:Save){const f=s.flags||{};const day=Math.max(1,n(s.day,1));const start=Math.max(0,n(f.pregnancyStartDay,n(f.pregnancyPossibleDay,n(f.pregnancyConfirmedDay,0))));const cycle=Math.max(1,n(f.pregnancyCycle,1));const laborCycle=Math.max(0,n(f.laborPregnancyCycle,0));const confirmed=f.pregnancyConfirmed===true||String(f.familyState||'')==='pregnant';return confirmed&&f.inLabor===true&&!!start&&day-start>=245&&laborCycle===cycle}
...
export function registerChild(input:{birthDay?:number;name?:string;sex?:'girl'|'boy'|'unknown'}={}){const s=read();if(!s||!canonicalBirthOpen(s))return null;const f=s.flags||(s.flags={});const records=getRecords(s);const birthDay=Math.max(1,n(input.birthDay,n(s.day,1)));const cycle=Math.max(1,n(f.pregnancyCycle,1));if(n(f.lastCanonicalBirthCycle,0)===cycle)return null;const r=makeRecord(records.length,birthDay);if(input.name?.trim())r.name=input.name.trim();if(input.sex)r.sex=input.sex;records.push(r);f.childRecords=records;s.children=records.length;f.lastBirthDay=birthDay;s.eventHistory=[...(s.eventHistory||[]),`child-registered:${r.id}:${birthDay}`].slice(-420);write(s);return r}

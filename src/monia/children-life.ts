...
export function ensureChildRecords(){const s=read();if(!s)return[] as ChildLifeSnapshot[];const count=Math.max(0,Math.floor(n(s.children,0)));const f=s.flags||(s.flags={});let records=getRecords(s);let changed=false;
  while(records.length<count){const index=records.length;const explicit=n(f[`child:${index+1}:birthDay`],0);const fallback=n(f.lastBirthDay,n(f.birthDay,n(s.day,1)));records.push(makeRecord(index,Math.max(1,explicit||fallback)));changed=true}
  if(records.length>count&&count>=0){records=records.slice(0,count);changed=true}
  if(changed){f.childRecords=records;s.eventHistory=[...(s.eventHistory||[]),`children-records-sync:${count}:${n(s.day,1)}`].slice(-420);write(s)}

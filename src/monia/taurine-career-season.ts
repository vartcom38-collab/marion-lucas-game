...
export function getTaurineCareerSeason():TaurineCareerSeason|null{
  const s=read();if(!s)return null;const annual=getAnnualLifeProfile();const seasonal=getSeasonalLifeSnapshot();const year=annual?.lifeYear||Math.floor((Math.max(1,n(s.day,1))-1)/365)+1;const roll=hash(`taurine-season:${year}`)%100;
  ...
  const careerDelta=annual?Math.round((annual.careerBias-50)/3):0;
  return{lifeYear:year,intensity,rhythm,targetDensity:Math.max(8,Math.min(95,base+seasonalDelta+careerDelta)),repeatCitiesAllowed:true,repeatFeriasAllowed:true,cityNoveltyIsOptional:true,principle:'Les mêmes villes et les mêmes ferias peuvent revenir plusieurs années. Ce sont le rythme, le cartel, le résultat, la fatigue, les déplacements et la vie autour qui doivent varier, pas la géographie à tout prix.'};
}

...
export function getLucasHomeRhythm():LucasHomeRhythm|null{
  const s=read();if(!s)return null;const f=s.flags||{};const together=f.lucasWithMarion===true||f.travelWithLucas===true;return resolveLucasHomeRhythm(String(s.place||''),together);
}

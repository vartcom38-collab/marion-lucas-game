...
export function getSpainLifeSnapshot():SpainLifeSnapshot|null{
  ...
  const socialOpen=physicallyInSpain&&(stage!=='arriving'||sinceArrival>=1);
  const homeRoutineOpen=physicallyInSpain&&(stage!=='arriving'||sinceArrival>=2);
  const independentLifeOpen=physicallyInSpain&&(stage==='rooted'||sinceArrival>=4);

  if(physicallyInSpain&&stage==='arriving')directions.push({id:'spain-bearings',label:'Prendre mes repères sans me presser',intent:'open-map',kind:'self',weight:74,minutes:60,reason:'Une arrivée doit laisser de la place à l’observation avant de remplir l’agenda.'});
  ...
  const reason=!physicallyInSpain&&rooted?'L’Espagne reste une base importante, mais Marion n’y est pas physiquement aujourd’hui.':stage==='arriving'?'Marion vient d’arriver: découverte et adaptation passent avant la surcharge sociale.':stage==='settling'?'La vie espagnole commence à prendre sa propre forme.':'L’Espagne est maintenant un vrai quotidien, avec une vie de couple mais aussi une vie propre à Marion.';
  return{active:physicallyInSpain||rooted,stage,place,directions:directions.sort((a,b)=>b.weight-a.weight),socialOpen,homeRoutineOpen,independentLifeOpen,reason};
}

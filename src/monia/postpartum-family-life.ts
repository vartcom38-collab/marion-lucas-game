...
function completePostpartumIfReady(s:Save,start:number,days:number){
  ...
  f.lastPregnancyStartDay=n(f.pregnancyStartDay,n(f.pregnancyPossibleDay,0))||undefined;f.lastPregnancyConfirmedDay=n(f.pregnancyConfirmedDay,0)||undefined;f.lastPostpartumCompletedDay=Math.max(1,n(s.day,1));f.postpartum=false;f.familyState='parenting';f.pregnancyConfirmed=false;f.pregnancyPossible=false;f.inLabor=false;delete f.birthDay;delete f.postpartumStartDay;delete f.postpartumRecoveryUntilDay;delete f.pregnancyPossibleDay;delete f.pregnancyTestEarliestDay;delete f.pregnancyConfirmedDay;delete f.pregnancyStartDay;s.eventHistory=[...(s.eventHistory||[]),`postpartum-complete:${start}:${Math.max(1,n(s.day,1))}`].slice(-420);write(s);return true
}

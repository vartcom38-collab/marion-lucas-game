...
function normalizePrimaryForCohabitation(state:PropertyState,s:Save|null){const f=s?.flags||{};const movedToMadrid=Boolean(f.livingWithLucas||f.movedToMadrid||f.cohabitingMadrid);if(!movedToMadrid||hasJointFinca(state))return state;const madrid=state.owned.find(p=>p.id==='lucas-madrid-house');if(madrid?.use==='primary')return state;for(const p of state.owned){if(p.use==='primary')p.use='available'}if(madrid)madrid.use='primary';return state}
...
export function submitPropertyOffer(offerIdValue:string,mode:'asking'|'careful'|'firm'='careful'){
  const s=readSave();if(!s)return null;const state=readState(),offer=availableOffers(state,s).find(o=>o.id===offerIdValue);if(!offer||!hasVisitedOffer(offerIdValue))return null;
...
export function acceptPropertyCounter(offerIdValue:string){const state=readState();const current=negotiationFor(state,offerIdValue);if(!current||current.status!=='countered'||!current.counterPrice)return false;current.acceptedPrice=current.counterPrice;current.status='accepted';state.negotiations={...(state.negotiations||{}),[offerIdValue]:current};return writeState(state)}
export function purchaseProperty(offerIdValue:string,makePrimary=false){
  const s=readSave();if(!s)return false;const state=normalizePrimaryForCohabitation(readState(),s),offer=availableOffers(state,s).find(o=>o.id===offerIdValue),negotiation=negotiationFor(state,offerIdValue);if(!offer||!hasVisitedOffer(offerIdValue)||!negotiation||negotiation.status!=='accepted'||!negotiation.acceptedPrice)return false;
  const firstJoint=!hasJointFinca(state);if(firstJoint&&!offer.firstHomeEligible)return false;if(makePrimary)for(const p of state.owned)if(p.use==='primary')p.use='available';
...
window.__moniaProperties=getPropertyLifeSnapshot;window.__moniaOpenPropertySearch=openPropertySearch;window.__moniaClosePropertySearch=closePropertySearch;window.__moniaRefreshPropertyOffers=refreshPropertyOffers;window.__moniaShortlistProperty=shortlistProperty;window.__moniaSubmitPropertyOffer=submitPropertyOffer;window.__moniaAcceptPropertyCounter=acceptPropertyCounter;window.__moniaPurchaseProperty=purchaseProperty;window.__moniaSetPrimaryProperty=setPrimaryProperty;window.__moniaSellProperty=sellProperty;

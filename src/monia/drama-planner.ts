...
function videoPrompt(request: MonIADramaRequest, shot: Omit<MonIADramaShot,'generationPrompt'>) {
  return `Photorealistic cinematic micro-drama, ${request.format || '9:16'}, ${request.context.place}, ${shot.focusActor}, ${shot.shotSize} shot, emotion ${shot.emotion}, ${shot.action}, ${shot.reaction}, camera ${shot.cameraMove}, ${shot.lighting}. Keep character appearance, wardrobe, hairstyle and location consistent with adjacent shots. Natural micro-expressions and motion. No text, captions, logos or watermarks.`;
}
...
export async function planDrama(request: MonIADramaRequest, enabled = true): Promise<MonIADramaPlan> {
  ...
  const result = await monia.direct({
    ...
    context: {
      ...
      rules: [...(request.context.rules || []), 'Préparer un storyboard vidéo court.', 'Ne jamais inventer un événement futur majeur.', 'Lucas reste absolument fidèle.', 'Dialogue bref et réactions naturelles.'],
    },
